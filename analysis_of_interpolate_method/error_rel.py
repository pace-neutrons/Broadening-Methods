# investigate error relationship and how this changes with sigma spacing
import numpy as np
...
sigma_spacing = np.linspace(4,np.sqrt(2),10)
...
ax11.plot(x, ydata, label='actual')
...
for space in sigma_spacing:
    ...
    ax2.plot([space],[area],'.',color='C1')

    ax3.plot([space],[k],'.',color='C2')

    ax11.plot(x, est, label='sigma=' + str(round(space,4)))
    ax12.plot(x, diff, label='sigma=' + str(round(space,4)))

ax12.legend()
ax12.legend()

sigma_max = 2
sigma_vals = np.linspace(sigma_min, sigma_max, 51)
